...
def ensure_env_file_exists():
    """
    Create .env file on first run - interactive in terminal, guided in GUI.

    Returns:
        bool: True if .env exists and ready to use, False if setup needed
    """
    import sys

    env_file = os.path.expanduser('~/.snap/.env')
    ...
    if not os.path.exists(env_file):
        ...
        if sys.stdin.isatty():
            ...
            print("🔧 SnapTask First-Time Setup")
            print("=" * 60)
            ...
            with open(env_file, 'w') as f:
                f.write('# SnapTask Configuration\n')
                f.write('# Generated by SnapTask setup\n\n')
                f.write(f'OPENAI_API_KEY={api_key}\n')

            os.chmod(env_file, 0o600)
            print(f"✅ Configuration saved to {env_file}")
            print("You can now use SnapTask!")
            print()
            return True

        else:
            # GUI MODE: Create placeholder + notification
            with open(env_file, 'w') as f:
                f.write('# SnapTask Configuration\n')
            ...
            show_notification(
                "SnapTask Setup Required",
                f"Created {env_file} - Edit this file to add your API key, then try again"
            )
            return False
    ...
    if not api_key or api_key == 'your-api-key-here':
        ...
        if sys.stdin.isatty():
            ...
            print(f"   Please edit {env_file} and add your API key.")
            return False
        else:
            show_notification(
                "SnapTask Setup Incomplete",
                f"Please edit {env_file} and replace 'your-api-key-here' with your actual API key"
            )
            return False
...
def load_env_config():
    """
    Load environment variables from ~/.snap/.env file.

    This allows the binary to read OPENAI_API_KEY from a config file
    when run from GUI apps (Shortcuts, Automator) that don't have
